...
async def answer_question(query: str):
    ...
    article_text = get_article_text(
        f"https://radiopaedia.org{all_search_results[r.id]['href']}"
    )
    ...
    Don't start your answer with something like 'Based on the context...'. Return the answer directly.
    ...
    def SubmitForm():
        return Form(
            ...
            **{"hx-on::after-request": "this.reset()"},
            ...
            # Button(
            #     Heroicon("arrow-up-circle", cls="w-6 h-6"),
            #     "Search",
            #     type="submit",
            #     cls="hidden flex gap-2 font-semibold bg-zinc-200 p-2 rounded-sm text-zinc-800 border hover:opacity-70 border-zinc-300",
            # ),
        )

    return Div(cls="max-w-[600px] mx-auto p-4 max-h-screen flex flex-col")(
        H1(cls="text-3xl font-bold text-zinc-800 mb-4 text-center")("RadiopaediaAI"),
        Div(
            ...
            Div(id="result", cls="rounded overflow-auto flex-1 min-h-0")(
                P(
                    "Ask any radiology related question below. RadiopaediaAI will try to answer it and give you a source for its answer.",
